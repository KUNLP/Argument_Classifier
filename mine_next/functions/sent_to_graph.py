...
def constituent_to_tree(tokenizer, constituent_string, sentence, word_offset, node_offset, num_orders=2):
    ...
    for i in range(1, num_orders):
        ...
        for idx, start, end, type, parent_idx in high_order_sequence[-1]:
            if parent_idx == -1:
                new_constituent_sequence.append((idx, start, end, type, parent_idx))
                continue
...
def final_graph(constituent_list, first_graph, second_graph):
    ...
    two_order_sent_cons_idx = [idx[0] for idx in two_order_sent_cons]
    ...
    for idx, start, end, label, parent_idx in two_order_sent_cons:
        ...
        if parent_idx != -1:
            ...
            second_graph.add_edges(idx_nodeid, parent_idx_nodeid,
                            data={'cc_link': torch.tensor([1]),
                                  'dtype': torch.tensor([1])})
    second_graph = dgl.add_self_loop(second_graph)
    ...
    constituent_labels_second = torch.tensor(constituent_labels_first, dtype=torch.long) # 라벨 개수 동일하게
...
def all_process_graph(nlp, tokenizer, sentence):
    ...
    claim_first_graph, claim_second_graph, constituent_labels_first, constituent_labels_second = \
        final_graph(constituent, first_graph, second_graph)
    return claim_first_graph, claim_second_graph, constituent_labels_first, constituent_labels_second
...
if __name__ == "__main__":
    ...
    test_data = pd.read_csv('../../data/IAM/claims/test.txt', sep='\t', header=None, quoting=csv.QUOTE_NONE)
    test_data.columns = ['claim_label', 'topic_sentence', 'claim_sentence', 'article_id', 'stance_label']
    test_data = test_data.dropna(axis=0)

    # train_sentences = train_data['claim_sentence'].tolist()
    # dev_sentences = dev_data['claim_sentence'].tolist()
    test_sentences = test_data['claim_sentence'].tolist()
    ...
    for idx, test in tqdm(enumerate(test_sentences), total=len(test_sentences)):
        test = test.lower().replace('“', '"').replace('”', '"')
        test = "".join(filter(lambda x : x in printable, test))
        test_first_graph, test_second_graph, test_constituent_labels_first, test_constituent_labels_second = \
            all_process_graph(nlp, tokenizer, test)
        #dgl.save_graphs('../../data/IAM/claims/graphs/test_first_graph_{}.dgl'.format(idx), test_first_graph)
        #dgl.save_graphs('../../data/IAM/claims/graphs/test_second_graph_{}.dgl'.format(idx), test_second_graph)
        total_test.append([test_constituent_labels_first.tolist(), test_constituent_labels_second.tolist()])

    # for idx, train in tqdm(enumerate(train_sentences), total=len(train_sentences)):
    #     # train = train.lower().replace("\xa0", '').replace('...', '').replace('—', ' ').replace('“', '').replace('’', "'").strip()
    ...
    # with open('../../data/IAM/claims/graphs/train_constituent_first_second.txt', 'w', encoding='utf-8') as f:
    #     for line in total_train:
    #         f.write(str(line)+'\n')
    #
    # with open('../../data/IAM/claims/graphs/dev_constituent_first_second.txt', 'w', encoding='utf-8') as f:
    #     for line in total_dev:
    #         f.write(str(line)+'\n')

    # with open('../../data/IAM/claims/graphs/test_constituent_first_second.txt', 'w', encoding='utf-8') as f:
