...
def do_train(config, model, optimizer, scheduler, train_dataloader, epoch, global_step):
    ...
    for step, batch in tqdm(enumerate(train_dataloader), desc='do_train(epoch_{})'.format(epoch), total=len(train_dataloader)):
        ...
        # )
        # base
        idx, input_ids, attention_mask, token_type_ids, stance_labels = batch[0], batch[1], batch[2], batch[3], \
                                                                             batch[4]
        loss, predicts = model(
            idx=idx,
            input_ids=input_ids,
        ...
        labels = stance_labels.cpu().detach().numpy().tolist()
...
def do_evaluate(model, dev_dataloader):
    ...
    for step, batch in tqdm(enumerate(dev_dataloader), desc="do_evaluate", total=len(dev_dataloader)):
        ...
        labels = stance_labels.detach().cpu().tolist()
        total_predicts.extend(predicts)
        total_corrects.extend(labels)
    target_names = ['class 0', 'class 1']
    print(classification_report(total_corrects, total_predicts, target_names=target_names, digits=4))
...
def train(config, model, tokenizer):
    ...
    # 데이터셋 로드
    train_data = pd.read_csv(config.stance_train, sep='\t', header=None, quoting=csv.QUOTE_NONE)
    #train_data.columns = ['claim_label', 'topic_sentence', 'claim_sentence', 'article_id', 'stance_label']
    train_data.columns = ['claim_label', 'topic_sentence', 'claim_sentence', 'id', 'stance_labels']
    train_data = train_data.dropna(axis=0)
    # train_data = train_data[:100]
    dev_data = pd.read_csv(config.stance_dev, sep='\t', header=None, quoting=csv.QUOTE_NONE)
    dev_data.columns = ['claim_label', 'topic_sentence', 'claim_sentence', 'id', 'stance_labels']
    ...
    train_dataset = convert_stance_data2tensordataset(train_data, tokenizer, config.max_length, 'train')
    dev_dataset = convert_stance_data2tensordataset(dev_data, tokenizer, config.max_length, 'dev')
    ...
    for epoch in range(config.epoch):
        ...
        train_accuracy, average_loss, global_step = do_train(
            ...
            train_dataloader=train_dataloader, epoch=epoch+1, global_step=global_step)
        print("train_accuracy : {}\taverage_loss : {}\n".format(round(train_accuracy, 4), round(average_loss, 4)))

        model.eval()
        test_accuracy, _ = do_evaluate(model=model, dev_dataloader=dev_dataloader)
